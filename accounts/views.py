...
def signup(request):
    if request.method == "POST":
        ...
        if request.POST['password1'] == request.POST['password2']:
                try:
                #looking for similar usernames already in the User object
...
def login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(request, 'accounts/login.html', {'error': 'Ooops! Something went wrong.'})
